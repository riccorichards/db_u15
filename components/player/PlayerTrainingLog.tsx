...
interface SessionLog {
  ...
  workRate: number; technicalQuality: number; tacticalAwareness: number;
  focusLevel: number; bodyLanguage: number; coachability: number;
  emotionalState: string; fatigueLevel: number; injuryFlag: boolean;
  minutesParticipated: number; prs: number; readinessLabel: string;
}

const EMOTION_EMOJI: Record<string, string> = {
  happy: "😄", neutral: "😐", tired: "😴", frustrated: "😤", anxious: "😟",
};

const SESSION_TYPE_COLORS: Record<string, string> = {
  tactical: "text-blue-300", physical: "text-red-300",
  technical: "text-purple-300", mixed: "text-sky", recovery: "text-green-300",
};

const READINESS_COLORS = {
  match_ready: "text-green-400", monitor: "text-yellow-400", rest: "text-red-400",
};

function ScoreDot({ value, inverse = false }: { value: number; inverse?: boolean }) {
...
export default function PlayerTrainingLog({ sessionLogs }: { sessionLogs: SessionLog[] }) {
  ...
  if (!sorted.length) {
    return (
      <div className="glass rounded-2xl p-5 flex items-center justify-center h-32">
        <p className="text-sky/40 text-sm font-body">No training sessions logged yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="glass rounded-2xl overflow-hidden">
      <div className="px-5 py-4 border-b border-sky/10 flex items-center justify-between">
        <div>
          <h3 className="font-display text-lg font-bold uppercase tracking-wider text-white">Training Log</h3>
          <p className="text-xs text-sky/40 font-body mt-0.5">
            All metrics explained: 1–10 scale · Fatigue is inverse (lower = better)
          </p>
        </div>
      ...
      {displayed.map((s, i) => (
        ...
        >
          <div className="text-xs font-mono text-sky/60">
            ...
            {s.injuryFlag && <AlertTriangle size={10} className="text-red-400 inline ml-1" />}
          </div>
          <div className={`text-[10px] font-mono capitalize ${SESSION_TYPE_COLORS[s.sessionType] ?? "text-sky/50"}`}>
            {s.sessionType}
          </div>
          <div className="text-center"><ScoreDot value={s.workRate} /></div>
          <div className="text-center"><ScoreDot value={s.technicalQuality} /></div>
          <div className="text-center"><ScoreDot value={s.tacticalAwareness} /></div>
          <div className="text-center"><ScoreDot value={s.focusLevel} /></div>
          <div className="text-center"><ScoreDot value={s.bodyLanguage} /></div>
          <div className="text-center"><ScoreDot value={s.coachability} /></div>
          <div className="text-center"><ScoreDot value={s.fatigueLevel} inverse /></div>
          <div className="text-center text-sm">{EMOTION_EMOJI[s.emotionalState] ?? "😐"}</div>
          <div className={`text-center text-xs font-mono font-bold ${READINESS_COLORS[s.readinessLabel as keyof typeof READINESS_COLORS] ?? "text-white"}`}>
      ...
      <div className="px-5 py-3 border-t border-sky/10 bg-navy-950/20">
        <p className="text-[10px] font-mono text-sky/30 mb-1">HOW METRICS ARE CALCULATED:</p>
        <div className="flex flex-wrap gap-x-4 gap-y-1">
          {[
            ...
            <div key={label} className="flex items-center gap-1">
              <span className="text-[10px] font-mono text-sky/50">{label}:</span>
