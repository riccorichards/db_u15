...
export default function PlayerMatchHistory({
  ...
  const played = [...matchPerformances]
    .filter((m) => m.minutesPlayed > 0)
    .reverse();
    ...
    <div className="glass rounded-2xl overflow-hidden">
      <div className="px-5 py-4 border-b border-sky/10 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-sky/40 font-body mt-0.5">
            {played.length} appearances this season
          </p>
        </div>
      </div>
      ...
      {played.map((m, i) => (
        ...
        >
          ...
          <div
            className={`text-center text-sm font-mono font-bold ${ratingColor(m.rating)}`}
          >
