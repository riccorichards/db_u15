...
const CustomDot = (props: { cx?: number; cy?: number; payload?: PRSPoint }) => {
  ...
  return (
    <circle
      ...
      cy={cy}
      r={4}
      fill={SESSION_COLORS[payload.sessionType] ?? "#018ABE"}
      stroke="#050d1a"
      strokeWidth={2}
    />
  );
...
const CustomTooltip = ({
  ...
  active?: boolean;
  payload?: { value: number; payload: PRSPoint }[];
  label?: string;
}) => {
  if (active && payload?.length) {
    ...
    return (
      <div className="glass rounded-xl px-3 py-2 text-xs border border-sky/10">
        <p className="text-sky/60 font-body mb-1">
...
export default function PlayerPRSTrend({ prsTrend }: { prsTrend: PRSPoint[] }) {
  ...
  const avg = Math.round(
    prsTrend.reduce((a, p) => a + p.prs, 0) / prsTrend.length,
  );
  const latest = prsTrend[prsTrend.length - 1].prs;
  const trend =
    prsTrend.length > 1 ? latest - prsTrend[prsTrend.length - 2].prs : 0;
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-sky/40 font-body mt-0.5">
            Player Readiness Score per session
          </p>
        </div>
